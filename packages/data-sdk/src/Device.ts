...
export interface IRealtimeDevice {
    ...
    startListeningToRealtimeDataStream(
        stream: RealtimeDataStream
    ): Promise<void>;
    stopListeningToRealtimeDataStream(
        stream: RealtimeDataStream
    ): Promise<void>;
...
export class Device implements IRealtimeDevice {
    ...
    getRealtimeStatus(): "disconnected" | "connecting" | "connected" {
        if (this.rtcClient && this.remoteDevicePeerId) {
            ...
            throw new Error(
                `Realtime connection hasn't been started for ${this.id}`
            );
        }
    }
    ...
    async createCaptureStream(streamName: string) {
        const result = await fetch(
            `${FORMANT_API_URL}/v1/admin/capture-sessions`,
            {
                method: "POST",
                body: JSON.stringify({
                    deviceId: this.id,
