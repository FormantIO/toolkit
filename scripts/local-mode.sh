...
function show_help() {
    ...
    echo "  INFLUX_USERNAME          InfluxDB username"
    echo "  INFLUX_PASSWORD          InfluxDB password"
...
if ! docker ps -a | grep $CONTAINER_NAME; then
     ...
     if [[ -z $INFLUX_PASSWORD ]]; then
          echo "ERROR: Please set the INFLUX_PASSWORD environment variable."
          do_clean
          exit 1
     fi
     ...
     if [[ "$1" == "shell" ]]; then
          ...
          printf "export FORMANT_INFLUX_TOKEN=${INFLUXDB_TOKEN}\nexport FORMANT_INFLUX_ORG=${INFLUX_ORGANIZATION}\nexport FORMANT_LOCAL=true\nexport FORMANT_INFLUX_URL=http://localhost:${INFLUX_PORT}\n\n"
     else
          FORMANT_INFLUX_TOKEN=${INFLUXDB_TOKEN} \
          FORMANT_INFLUX_ORG=${INFLUX_ORGANIZATION} \
          FORMANT_LOCAL=true \
          FORMANT_INFLUX_URL=http://localhost:8086
     fi
else
