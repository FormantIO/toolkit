import { AudioPlayer, Authentication, Fleet } from "@formant/data-sdk";
...
el("button").addEventListener("click", async () => {
  try {
    ...
    const audioStreams = await device.getRealtimeAudioStreams();
    if (audioStreams.length > 0) {
      log("Audio streams: " + JSON.stringify(audioStreams));
      log(
        "Starting to listen to audio stream " + audioStreams[0].name + " ... "
      );

      const audioPlayer = new AudioPlayer(device, audioStreams[0]);
      audioPlayer.play();
    }
